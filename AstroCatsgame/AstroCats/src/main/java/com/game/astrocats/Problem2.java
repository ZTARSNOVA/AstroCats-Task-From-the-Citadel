...
public class Problem2 extends JPanel {
    ...
    public Problem2(ProblemHandler handler) {
    ...
    private void createContent() {
        ...
        JLabel titleLabel = new JLabel("Problem 2: Optimize Orbital Jump");
    ...
    private JPanel createContextPanel() {
        ...
        String html = "<html><body style='font-size: 16px; color: white;'>"
            + "<b>Context:</b> To escape the planet, the ship must jump using fuel efficiently.<br/>"
            + "If a segment uses more than 10 fuel units, cap it at 10. Sum the optimized values.<br/><br/>"
            + "<b>Example 1:</b><br/>Input: [7, 15, 4] → Output: 21<br/>"
            + "<b>Example 2:</b><br/>Input: [12, 3, 9, 11] → Output: 32<br/>"
            + "</body></html>";
    ...
    private JPanel createEditorPanel() {
        ...
        codeEditor = new JTextArea("def optimize_orbital_jump(fuel_segments):\n    # Code here\n    return 0\n");
    ...
    private class RunButtonListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            ...
            boolean hasFunction = code.contains("def optimize_orbital_jump(fuel_segments):");
            boolean hasCorrectLogic = code.contains("sum(") && code.contains("min(") && code.contains(", 10)");

            if (hasFunction && hasCorrectLogic) {
                resultLabel.setForeground(new Color(0, 200, 100));
                resultLabel.setText("✅ Optimized fuel calculation is correct!");
                if (problemHandler != null) {
                    problemHandler.onProblemCompleted();
                }
            } else {
                resultLabel.setForeground(new Color(200, 50, 50));
                resultLabel.setText("❌ Incorrect logic. Remember: cap each segment at 10.");
            }
        }
    }
}
